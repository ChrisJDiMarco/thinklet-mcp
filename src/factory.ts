/**
 * createMcpServer — factory that wires up all Thinklet tools and prompts on a
 * fresh McpServer instance. Called once per stdio session, and once per HTTP
...
import { z } from "zod";
import { ApiClient, AuthError, type ThinkletMeta, type SkillMode } from "./api.js";
import { THINKLET_SKILL } from "./skill.js";
import { getCatalog, getIntegrationDocs, matchTriggers } from "./integrations.js";

// ─── Viewer HTML (loaded once at module level — same binary for all sessions) ─
...
try {
  ...
  viewerHtml =
    "<html><body><p>Viewer not built. Run npm run build:ui.</p></body></html>";
}
...
function buildToolResult(
  ...
  if (Object.keys(sc).length > 0) result.structuredContent = sc;
  return result;
}
...
function summaryText(meta: ThinkletMeta) {
  return [
    ...
    meta.score != null ? `Relevance: ${meta.score}%` : "",
  ]
    .filter(Boolean)
    .join("\n");
}
...
export function createMcpServer(api: ApiClient): McpServer {
  const server = new McpServer({ name: "thinklet", version: "0.5.0" });
  ...
  function resolveAppUrl(meta: ThinkletMeta): string {
    return api.buildAppUrl(meta.id, meta.visibility, meta.platformUrl, meta.publicationId);
  }
      ...
      if (ids && ids.length > 0 && includeDocs) {
        ...
        return {
          content: [
            {
              type: "text",
              text: [`📚 **Integration docs for:** ${ids.join(", ")}`, "", docs].join(
                "\n"
              ),
            },
          ],
        ...
        content: [
          {
            ...
            text: [
              ...
              "",
              ...catalog.map((i) => `- **${i.name}** (\`${i.id}\`) — ${i.description}`),
            ].join("\n"),
          },
        ],
      };
  ...
  server.tool(
    ...
    async ({ ids }) => {
          ...
          content: [
            {
              type: "text",
              text: [
                `❌ **Unknown integration(s): ${invalidIds.map((id) => `\`${id}\``).join(", ")}**`,
                ``,
                `These do not exist on the Thinklet platform. The ONLY available integrations are:`,
                ``,
                catalogList,
                ``,
                `Call \`confirm_integrations\` again with valid IDs from the list above, or \`[]\` for none.`,
              ].join("\n"),
            },
          ],
          isError: true,
      ...
      if (ids.length === 0 && buildFlow.recommendedIntegrations.length > 0) {
        const recommended = catalog.filter((i) =>
          buildFlow.recommendedIntegrations.includes(i.id)
        );
        const names = recommended.map((i) => `**${i.name}**`).join(", ");
        return {
          content: [
            {
              type: "text",
              text: [
                `⚠️ **No integrations selected — but your request needs ${names} to work.**`,
                ``,
                `Without ${names}, the thinklet won't be able to do what the user asked.`,
                ``,
                `Please ask the user to confirm:`,
                `- Add the recommended integration(s) and proceed?`,
                `- Or proceed without any integrations (thinklet will have no AI/media capability)?`,
                ``,
                `Then call \`confirm_integrations\` again with their choice.`,
              ].join("\n"),
            },
          ],
          isError: true,
  ...
  server.tool(
    ...
    [
      ...
      "If the user asks 'what is Thinklet' or similar, introduce it as: Thinklet is a platform for AI-built interactive apps — single-file React components that run in the browser with built-in AI, data, and media capabilities. Browse and build at app.thinklet.io.",
      "If the user's request does not mention 'thinklet' explicitly, briefly say: 'I'll build this as a Thinklet — a shareable interactive app on thinklet.io.' then proceed immediately.",
    ...
    async ({ query, limit, includePrivate }) => {
      ...
      integrationSection.push(
        `⛔ **PLATFORM CONSTRAINT — CRITICAL:** The integrations listed above are the COMPLETE and ONLY integrations available on this platform.`,
        `Do NOT suggest, mention, or use any integration, library, or API not in this list (e.g. no Stripe, Firebase, Twilio, FFmpeg, or any external service).`,
        `If the user asks for something not in this list, tell them it is not available and offer the closest match from the list above.`,
        ``
      );
      ...
      const actionBlock = [
        ...
        ``,
        `**After the user replies — follow the correct path:**`,
        ``,
        `🔀 **If REMIX:** You MUST call \`get_thinklet({ id: "<chosen_id>", includeCode: true })\` FIRST`,
        `to fetch the source code of the thinklet being remixed. Do this BEFORE \`confirm_integrations\`.`,
        `You cannot write a remixed version without reading the original code.`,
        `Then call \`confirm_integrations\`, generate the modified code, and call \`save_thinklet\`.`,
        ``,
        `🆕 **If FRESH:** Skip \`get_thinklet\` and go straight to \`confirm_integrations\`.`,
        ``,
        `**In both cases your next mandatory call after getting the user's reply is:**`,
      ...
      try {
        const results = await api.searchThinklets(
          query,
          limit ?? 5,
          includePrivate ?? true
        );
        ...
        if (publishedResults.length > 0) {
          sections.push(
            `**Published Thinklets**`,
            ``,
            formatCards(publishedResults, nextIndex)
          );
        ...
        if (err instanceof AuthError) return authErrorResponse();
        buildFlow.discovered = true;
        buildFlow.integrationsConfirmed = false;
        buildFlow.lastUpdated = Date.now();
        const msg = err instanceof Error ? err.message : String(err);
  ...
  registerAppTool(
    ...
    "save_thinklet",
    {
      ...
      inputSchema: {
        ...
        title: z
          .string()
          .describe(
            "Short descriptive title (STRICT MAX 60 chars, 5 words or less). Will be truncated if longer."
          ),
        description: z
          .string()
          .describe(
    ...
    async ({ code, title, description: rawDesc, tags, integrations }) => {
      ...
      try {
        const meta = await api.createThinklet({
          code,
          title: safeTitle,
          description,
          tags,
  ...
  registerAppTool(
    ...
    async ({ id, includeCode }) => {
      try {
        const meta = await api.getThinklet(id);
        const thinkletCode = await api.getThinkletCode(id).catch(() => "");
        const url = resolveAppUrl(meta);

        const embedUrl = await api
          .fetchAccess(id)
          .then((access) => api.buildEmbedUrl(access.contentApiToken))
          .catch((err) => {
            process.stderr.write(
        ...
        if (includeCode && thinkletCode) {
          buildFlow.codeRead = true;
          buildFlow.lastUpdated = Date.now();
        }

        const codeText =
          includeCode && thinkletCode
            ? `\n\n---\n\n**Source code:**\n\`\`\`jsx\n${thinkletCode}\n\`\`\``
            : "";
        ...
        const lines = [
          ...
          ``,
          thinkletCode
            ? `🖼️ **Inline preview attached below.**`
            : `🧭 **Open in Thinklet fallback attached below.**`,
          codeText,
        ];
  ...
  registerAppTool(
    ...
    "fix_thinklet",
    {
      description: [
        ...
        "MANDATORY: You MUST call `get_thinklet` with `includeCode: true` before calling this — never fix code you haven't read.",
        "Ask the user to paste the error message from the platform if they haven't already — you cannot see browser errors.",
        "Provide the COMPLETE updated code — this replaces the existing version in place (same URL, same ID).",
        "IMPORTANT: You MUST include the app URL from the response in your message to the user.",
      ].join(" "),
      inputSchema: {
        ...
        title: z
          ...
          .optional()
          .describe(
            "Updated title, MAX 60 chars (optional — keeps existing if omitted). Truncated if longer."
          ),
        description: z
          .string()
          .optional()
          .describe(
            "Updated description, MAX 200 chars (optional — keeps existing if omitted). Truncated if longer."
          ),
        tags: z
          .array(z.string())
          .optional()
    ...
    async ({ id, code, title: rawTitle, description: rawDesc, tags, integrations }) => {
      if (!buildFlow.codeRead) {
        return {
          content: [
            {
              type: "text" as const,
              text: [
                `❌ **Cannot fix without reading the code first.**`,
                ``,
                `Call \`get_thinklet({ id: "${id}", includeCode: true })\` first to fetch the current source code.`,
                `Also ask the user to paste the error message from the platform if they haven't already.`,
                `Then call \`fix_thinklet\` with the complete corrected code.`,
              ].join("\n"),
            },
          ],
          isError: true,
      ...
      const title =
        rawTitle && rawTitle.length > 60
          ? rawTitle.slice(0, 57) + "..."
          : rawTitle;
      const description =
        rawDesc && rawDesc.length > 200
          ? rawDesc.slice(0, 197) + "..."
          : rawDesc;

      try {
        const meta = await api.fixThinklet(
          id,
          code,
          title,
          description,
  ...
  registerAppTool(
    ...
    async ({ id, visibility }) => {
      try {
        const meta = await api.setVisibility(id, visibility);
        const thinkletCode = await api.getThinkletCode(id).catch(() => "");
        const url = resolveAppUrl(meta);
        ...
        const lines = [
          ...
          `✅ **Visibility changed!**`,
          summaryText(meta),
          visibilityExplainer(visibility),
          ``,
          thinkletCode
            ? `🖼️ **Inline preview attached below.**`
            : `🧭 **Open in Thinklet fallback attached below.**`,
